...
if (!isset($_SESSION['tbl']) and !isset($_SESSION['operazione'])){
    ...
    b. Determinare i vice primari che hanno sostituito almeno due volte il proprio primario</h1><br>";
  ...
  $conn = pg_connect("host=localhost port=5432 dbname=Ospedale user=postgres password=unimi");
   ...
   // echo $queryCount;
    ...
    echo '</tr></form>';
    echo '</table><br>';
    echo "<div><h1>Query eseguita:</h1> <h2 style='text-align:left'>select cfvice 
    from sostituzione 
    group by cfvice
    having count(*) >1
    </h3> </div>";
