...
if (!isset($_SESSION['tbl']) and !isset($_SESSION['operazione'])){
    ...
    b. Dello storico dei ricoveri di un paziente in una struttura </h1><br>";
  ...
  $conn = pg_connect("host=localhost port=5432 dbname=Ospedale user=postgres password=unimi");
    ...
    $query= "select ts, dataricovero, codospedale
    from lettoricoverato
    order by ts
   ...
   // echo $queryCount;
    ...
    echo '</tr></form>';
    echo '</table><br>';
    echo "<div><h1>Query eseguita:</h1> <h2 style='text-align:left'>select ts, dataricovero, codospedale <br>
    from lettoricoverato <br>
    order by ts
    </h3> </div>";
    echo "<div><br>";
